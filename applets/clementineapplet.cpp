...
void clementineApplet::update() {
    ...
    QDBusMessage m = QDBusMessage::createMethodCall((QString)"org.mpris.clementine",(QString)"/Player","",(QString)"GetMetadata");
    QDBusReply<QVariantMap> result = QDBusConnection::sessionBus().call(m);

    g15r_clearScreen(canvas, 0);

    if (!result.isValid()) {
        g15r_G15FPrint (canvas, (char *) "Clementine", 0, 0, G15_TEXT_HUGE, G15_JUSTIFY_CENTER, G15_COLOR_BLACK, 1);
